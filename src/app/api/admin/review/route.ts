...
import { reviewDrafts, reviewQueueItem } from "@/lib/growth-machine/review";
...
export async function POST(req: NextRequest) {
    ...
    try {
        const body = (await req.json()) as ReviewPostBody;

        if (body.queueId) {
            const result = await reviewQueueItem(body.queueId);
            return NextResponse.json({
                success: true,
                result,
            });
        ...
        const results = await reviewDrafts(limit);
