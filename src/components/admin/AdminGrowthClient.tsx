...
export default function AdminGrowthClient() {
    ...
    return <div>
        ...
        <section style={{ ...grid, gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", marginBottom: 16 }}>
            <Panel title="Runs recents" meta={`${runs.length} charge(s)`}>{runs.length ? <div style={{ ...grid, gap: 10 }}>{runs.map((run) => <div key={run.id} style={{ border: "1px solid #ececec", borderRadius: 8, background: "#fafafa", padding: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 8 }}><strong style={{ fontSize: 13 }}>{run.run_label || run.trigger_source || "run"}</strong><span style={statusPill(run.status)}>{run.status}</span></div><div style={muted}>{formatDate(run.created_at)}</div><div style={muted}>briefs {statValue(run.stats, "briefCount")} · drafts {statValue(run.stats, "contentDraftCount")} · reviews {statValue(run.stats, "reviewCount")}</div><div style={muted}>cluster {run.cluster_ref || "general"} · erreurs {Array.isArray(run.errors) ? run.errors.length : 0}</div></div>)}</div> : <p style={muted}>Aucun run recent.</p>}</Panel>
        ...
        <div style={{ ...grid, gridTemplateColumns: "320px minmax(0,1fr)" }}>
            <aside style={card}>
                <div style={{ display: "grid", gap: 10, marginBottom: 14 }}>
                    ...
                    <label style={muted}><span>Review</span><select value={reviewFilter} onChange={(e) => setReviewFilter(e.target.value)} style={{ width: "100%", marginTop: 6, padding: "8px 10px", border: "1px solid #ddd", borderRadius: 6 }}><option value="all">Toutes</option><option value="approved_only">Approved only</option><option value="needs_review">Sans review</option></select></label>
            ...
            <section style={card}>
                {selectedItem ? <>
                    ...
                    <div style={{ ...grid, gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", marginBottom: 16 }}>
                        ...
                        <div style={{ ...card, padding: 14 }}><strong>Etat interne</strong><p style={muted}>draftPack: {draftPack ? "oui" : "non"}</p><p style={muted}>contentDraft: {draft ? "oui" : "non"}</p><p style={muted}>review: {review ? review.verdict || "oui" : "non"}</p><p style={muted}>promotion: {selectedItem.payload.promotion ? "oui" : "non"}</p></div>
                    </div>
